---
title: "Capítulo 10: Señales y análisis de Fourier — 8.03SC Física III: Vibraciones y Ondas"
...
<ul>
<li class="video-item">
<button type="button" class="video-play" data-vid="VGAlyJ7e0IQ" data-title="Clase 14: Transformada de Fourier, radio AM">
<span class="video-play-icon">▶</span> <strong>Clase 14: Transformada de Fourier, radio AM</strong>
</button>
<a class="video-ext" href="https://www.youtube.com/watch?v=VGAlyJ7e0IQ" target="_blank" rel="noopener">YouTube ↗</a>
</li>
<li class="video-item">
<button type="button" class="video-play" data-vid="In0E5_JrPpo" data-title="Clase 15: Principio de incertidumbre, ondas en 2D">
<span class="video-play-icon">▶</span> <strong>Clase 15: Principio de incertidumbre, ondas en 2D</strong>
</button>
<a class="video-ext" href="https://www.youtube.com/watch?v=In0E5_JrPpo" target="_blank" rel="noopener">YouTube ↗</a>
</li>
</ul>
...
ii. Discutimos, primero con un ejemplo y después en general, el contrapunto entre una función y su transformada de Fourier. Establecemos la conexión con los conceptos físicos de ancho de banda y fidelidad en la transmisión de señales, y con la relación de incertidumbre de Heisenberg en mecánica cuántica.

iii. Desarrollamos con cierto detalle un ejemplo de dispersión de un paquete de ondas.

iv. Discutimos con más detalle la relación de dispersión de las ondas electromagnéticas y exploramos la cuestión de si la luz viaja realmente a la velocidad de la luz.
...
$$\frac{\partial^2}{\partial t^2}\psi(x, t) = v^2\frac{\partial^2}{\partial x^2}\psi(x, t). \tag{10.2}$$
...
$$f(t) = \begin{cases} 1 - |t| & \text{para } |t| \leq 1 \\ 0 & \text{para } |t| > 1 \end{cases} \tag{10.8}$$

la forma de la cuerda en una sucesión de instantes se muestra en la figura 10.2 y está animada en el programa 10-1.
...
$$f(t) = \int_{-\infty}^{\infty} d\omega\, C(\omega)\,e^{-i\omega t}. \tag{10.9}$$
...
$$e^{-i\omega t} \to e^{-i\omega t + ikx}. \tag{10.13}$$
...
$$\psi(x, t) = \int d\omega\, C(\omega)\,e^{-i\omega t + ikx}, \tag{10.14}$$
...
$$\cos(k_+x - \omega_+t) + \cos(k_-x - \omega_-t) \tag{10.18}$$

donde

$$k_\pm = k_0 \pm k_s, \qquad \omega_\pm = \omega_0 \pm \omega_s, \tag{10.19}$$

para

$$k_s \ll k_0, \qquad \omega_s \ll \omega_0. \tag{10.20}$$

La suma puede escribirse como un producto de cosenos:

$$2\cos(k_sx - \omega_st)\cdot\cos(k_0x - \omega_0t). \tag{10.21}$$
...
$$\omega = \omega(k) = \omega_0 + (k - k_0)\left.\frac{\partial\omega}{\partial k}\right|_{k = k_0}, \qquad \omega_0 \equiv \omega(k_0), \tag{10.25}$$

y los términos de orden superior son despreciables para un rango de frecuencias

$$\omega_0 - \Delta\omega < \omega < \omega_0 + \Delta\omega, \tag{10.26}$$
...
$$f(t)\cdot e^{-i\omega_0 t} \tag{10.28}$$
...
$$\omega = vk + a, \qquad k = \omega/v + b, \tag{10.29}$$

donde $a$ y $b$ son constantes que podemos determinar a partir de (10.25),

$$a = \omega_0 - vk_0, \qquad b = k_0 - \omega_0/v, \tag{10.30}$$

y $v$ es la velocidad de grupo

$$v = \left.\frac{\partial\omega}{\partial k}\right|_{k = k_0}. \tag{10.32}$$
...
$$C(\omega) = \frac{1}{2\pi}\int_{-\infty}^{\infty} dt\, f(t)\,e^{i\omega t}. \tag{10.37}$$
...
$$1 = f(0) = \int d\omega\, C(\omega)\,e^{-i\omega\cdot 0} = \frac{1}{\pi}\int_{-\infty}^{\infty} d\omega\,\frac{\Gamma}{\Gamma^2 + \omega^2} = \frac{1}{\pi}\int_{-\pi/2}^{\pi/2} d\theta = 1. \tag{10.41}$$

Para obtener la transformada de Fourier inversa, (10.37), como límite de una serie de Fourier, conviene usar una condición de contorno ligeramente distinta de las que discutimos en el capítulo 6 (extremos fijos y extremos libres). En su lugar, consideremos una cuerda tensada de $x = -\pi\ell$ a $x = \pi\ell$, en la que suponemos que el desplazamiento de la cuerda respecto del equilibrio en $x = \pi\ell$ es el mismo que el desplazamiento en $x = -\pi\ell$:[^cilindro]

$$\psi(-\pi\ell, t) = \psi(\pi\ell, t). \tag{10.42}$$

[^cilindro]: Un ejemplo de sistema físico con este tipo de condición de contorno sería una cuerda tensada alrededor de un cilindro sin fricción de radio $\ell$ y, por tanto, de circunferencia $2\pi\ell$. Entonces (10.42) sería cierta porque $x = -\pi\ell$ describe el mismo punto de la cuerda que $x = \pi\ell$.
...
$$\psi(x) = \sum_{n = -\infty}^{\infty} c_n\,e^{-inx/\ell}. \tag{10.44}$$
...
$$f(t) = \sum_{n = -\infty}^{\infty} c_n\,e^{-int/T}, \tag{10.45}$$
...
$$\frac{1}{T} \to d\omega, \qquad \frac{n}{T} \to \omega, \qquad c_n T \to C(\omega). \tag{10.48}$$
...
$$2\pi C(\omega) = \int_{-\infty}^{\infty} dt\, e^{-\Gamma|t|}e^{i\omega t}. \tag{10.49}$$
...
$$2\pi C(\omega) = \frac{1}{\Gamma - i\omega} + \frac{1}{\Gamma + i\omega} = \frac{\Gamma + i\omega}{\Gamma^2 + \omega^2} + \frac{\Gamma - i\omega}{\Gamma^2 + \omega^2} = \frac{2\Gamma}{\Gamma^2 + \omega^2}, \tag{10.56}$$
...
$$\langle g(t)\rangle = \frac{\int_{-\infty}^{\infty} dt\, g(t)\,|f(t)|^2}{\int_{-\infty}^{\infty} dt\,|f(t)|^2}. \tag{10.57}$$
...
$$\left\langle[t - \langle t\rangle]^2\right\rangle \equiv \Delta t^2 \tag{10.58}$$
...
$$\langle\omega\rangle = \frac{\int_{-\infty}^{\infty} dt\, f(t)^*\,i\frac{\partial}{\partial t}f(t)}{\int_{-\infty}^{\infty} dt\,|f(t)|^2} \tag{10.60}$$

y

$$\Delta\omega^2 \equiv \left\langle[\omega - \langle\omega\rangle]^2\right\rangle = \frac{\int_{-\infty}^{\infty} dt\,\left|\left(i\frac{\partial}{\partial t} - \langle\omega\rangle\right)f(t)\right|^2}{\int_{-\infty}^{\infty} dt\,|f(t)|^2}. \tag{10.61}$$
...
$$\Delta t\,\Delta\omega \geq \frac{1}{2}. \tag{10.62}$$
...
$$\Delta t \geq \frac{1}{2\Delta\omega}. \tag{10.63}$$
...
Para demostrar (10.62), considere la función[^heisenberg]

$$\left([t - \langle t\rangle] - i\kappa\left(i\frac{\partial}{\partial t} - \langle\omega\rangle\right)\right)f(t) = r(t), \tag{10.64}$$

que depende del parámetro enteramente libre $\kappa$.

[^heisenberg]: Este es un truco tomado de un análisis similar que conduce al principio de incertidumbre de Heisenberg en mecánica cuántica. No se preocupe si no le resulta obvio de dónde sale. Lo importante es el resultado.

Ahora fíjese en el cociente

$$\frac{\int_{-\infty}^{\infty} dt\,|r(t)|^2}{\int_{-\infty}^{\infty} dt\,|f(t)|^2}. \tag{10.65}$$
...
$$\Delta t^2 + \kappa^2\Delta\omega^2 + \kappa\,\frac{\int_{-\infty}^{\infty} dt\, f(t)^*\left(t\frac{\partial}{\partial t} - \frac{\partial}{\partial t}t\right)f(t)}{\int_{-\infty}^{\infty} dt\,|f(t)|^2}. \tag{10.66}$$

Todos los demás términos se cancelan. Pero

$$\frac{\partial}{\partial t}\left[t f(t)\right] = f(t) + t\frac{\partial}{\partial t}f(t). \tag{10.67}$$

Así, el último término de (10.66) es simplemente $-\kappa$, y (10.65) queda

$$\Delta t^2 + \kappa^2\Delta\omega^2 - \kappa. \tag{10.68}$$
...
$$\Delta\omega = 2\pi\Delta\nu \approx 3\times10^4\ \text{s}^{-1}, \tag{10.71}$$
...
$$\Delta x\,\Delta k \geq \frac{1}{2}. \tag{10.72}$$
...
$$\Delta x\,\Delta p \geq \frac{\hbar}{2}. \tag{10.74}$$
...
$$\omega = \begin{cases} v_I k = \sqrt{\dfrac{T}{\rho_I}}\,k & \text{en la región I} \\[8pt] v_{II} k = \sqrt{\dfrac{T}{\rho_{II}}}\,k & \text{en la región II} \end{cases} \tag{10.75}$$
...
$$\psi(x, t) = \begin{cases} f(t - x/v_I) + g(t + x/v_I) & \text{en la región I} \\[4pt] h(t - x/v_{II}) & \text{en la región II} \end{cases} \tag{10.77}$$
...
$$f(t) + g(t) = h(t), \tag{10.78}$$

y

$$\left.\frac{\partial}{\partial x}\left[f(t - x/v_I) + g(t + x/v_I)\right]\right|_{x=0} = \left.\frac{\partial}{\partial x}h(t - x/v_{II})\right|_{x=0}. \tag{10.79}$$
...
$$-f'(t) + g'(t) = -\frac{v_I}{v_{II}}h'(t). \tag{10.81}$$

Derivando (10.78), obtenemos

$$f'(t) + g'(t) = h'(t). \tag{10.82}$$
...
$$\psi(x, t) = \begin{cases} f(t - x/v_I) + \dfrac{1 - v_I/v_{II}}{1 + v_I/v_{II}}f(t + x/v_I) & \text{en la región I,} \\[10pt] \dfrac{2}{1 + v_I/v_{II}}f(t - x/v_{II}) & \text{en la región II.} \end{cases} \tag{10.86}$$
...
$$\tau = \frac{2\Omega}{2\Omega - i\omega}, \qquad R = \frac{i\omega}{2\Omega - i\omega} \tag{10.90}$$
...
$$\psi_{\text{in}}(x - vt) = A e^{-\Gamma|t - x/v|}. \tag{10.92}$$
...
$$C_\tau(\omega) = \frac{A}{2\pi}\left(\frac{1}{\Gamma - i\omega} + \frac{1}{\Gamma + i\omega}\right)\frac{2\Omega}{2\Omega - i\omega} \tag{10.98}$$

$$C_R(\omega) = \frac{A}{2\pi}\left(\frac{1}{\Gamma - i\omega} + \frac{1}{\Gamma + i\omega}\right)\frac{i\omega}{2\Omega - i\omega} \tag{10.99}$$
...
$$C_\tau(\omega) = A\,\frac{1}{2\pi}\frac{2\Omega}{2\Omega - \Gamma}\left(\frac{1}{\Gamma - i\omega} - \frac{1}{2\Omega - i\omega}\right) + \frac{1}{2\pi}\frac{2\Omega}{2\Omega + \Gamma}\left(\frac{1}{\Gamma + i\omega} + \frac{1}{2\Omega - i\omega}\right); \tag{10.100}$$

$$C_R(\omega) = A\,\frac{1}{2\pi}\frac{1}{2\Omega - \Gamma}\left(\frac{\Gamma}{\Gamma - i\omega} - \frac{2\Omega}{2\Omega - i\omega}\right) + \frac{1}{2\pi}\frac{1}{2\Omega + \Gamma}\left(-\frac{\Gamma}{\Gamma + i\omega} + \frac{2\Omega}{2\Omega - i\omega}\right). \tag{10.101}$$
...
$$\theta(t) = \begin{cases} 1 & \text{para } t \geq 0, \\ 0 & \text{para } t < 0. \end{cases} \tag{10.104}$$

*(Figura 10.7: un paquete de ondas en una cuerda tensada, en $t = -2$.)*

*(Figura 10.8: $t = -1$.)*

Estas fórmulas no son muy transparentes ni informativas, pero podemos meterlas en un ordenador y mirar el resultado. Representaremos el resultado en el límite $2\Omega \to \Gamma$. Los resultados (10.102) y (10.103) parecen singulares en este límite pero, en realidad, el límite existe y es perfectamente suave.[^critico] En las figuras 10.7-10.12 mostramos $\psi(x, t)$ para $\Gamma = v = 1$ en unidades arbitrarias, para valores de $t$ desde $-2$ hasta 3.

[^critico]: La aparente singularidad es similar a una que aparece al aproximarse al amortiguamiento crítico, discutido en (2.12).
...
Lo realmente interesante ocurre entre $t = 0$ y $t = 1$, así que lo miraremos en una escala temporal más fina en las figuras 10.13-10.16. Para apreciarlo de verdad, debería verlo en movimiento: está animado en el programa 10-4.
...
$$E = \hbar\omega, \qquad p = \hbar k, \tag{10.105}$$
...
$$E^2 = p^2c^2 + m^2c^4, \qquad v = c\,\frac{pc}{E} \tag{10.106}$$
...
$$\omega^2 = c^2k^2 + \omega_0^2, \qquad \omega_0 = \frac{mc^2}{\hbar}. \tag{10.107}$$
...
Parecería que deberíamos poder comprobar esta idea observando dos fotones de frecuencias distintas emitidos al mismo tiempo desde un objeto lejano y comprobando si llegan al mismo tiempo. Hay un fallo evidente en este plan: si el objeto está tan lejos que no podemos llegar hasta él, ¿cómo sabemos que los dos fotones fueron emitidos al mismo tiempo? De hecho, la astrofísica nos ha proporcionado una manera de sortear esta dificultad: podemos mirar los púlsares. Los púlsares son (presumiblemente) restos de estrellas de neutrones en rotación procedentes de explosiones de supernova, que emiten luz hacia la Tierra a intervalos regulares. Por ejemplo, el púlsar 1937+21 es tan regular que el instante de salida de los fotones puede determinarse con una precisión de unos pocos microsegundos (µs).[^pulsar] Además está a unos 16 000 años luz, así que los fotones de mayor frecuencia (los más rápidos) tienen tiempo de sobra para adelantarse. Cuando se hace este experimento, se encuentra un $\omega_0$ no nulo, de unos $1.7\times10^4\ \text{s}^{-1}$, que corresponde a una masa de unos $1.26\times10^{-49}$ g. Puede parecer una masa bastante pequeña, pero de hecho es ridículamente grande para un fotón: por estudios del campo magnético galáctico sospechamos que es menor que $4\times10^{-65}$ g.[^chibisov] Así que está ocurriendo alguna otra cosa.

[^pulsar]: Véase G. Barbiellini y G. Cocconi, *Nature* **329** (1987) 21.

[^chibisov]: Chibisov, *Soviet Physics - Uspekhi*, **19** (1986) 624.
...
$$E_x(\vec{r}, t) = E_0 e^{i(kz - \omega t)}, \qquad B_y(\vec{r}, t) = B_0 e^{i(kz - \omega t)}, \tag{10.108}$$
...
$$F_x(\vec{r}, t) = e\,E_x(\vec{r}, t) = e\,E_0 e^{i(kz - \omega t)} = m\,a_x(\vec{r}, t). \tag{10.110}$$
...
$$a_x(\vec{r}, t) = -\omega^2 d_0 e^{i(kz - \omega t)}. \tag{10.112}$$
...
El movimiento de los electrones da lugar a una densidad de corriente:[^electrones]

$$J_x = \frac{i\,e^2 N\,E_0}{m\,\omega}e^{i(kz - \omega t)},$$

donde $N$ es la densidad numérica de electrones.

[^electrones]: Nótese que el resultado es inversamente proporcional a la masa del electrón. Por eso nos concentramos en los electrones y no en los protones: ¡los protones no se mueven tan deprisa!
...
$$\omega^2 = c^2k^2 + \omega_0^2, \qquad \text{con } \omega_0^2 = \frac{e^2 N}{m\,\varepsilon_0}. \tag{10.118}$$
...
i. Resolver un problema de oscilación forzada para una cuerda tensada con un desplazamiento arbitrario dependiente del tiempo en el extremo;

ii. Descomponer una señal arbitraria en componentes armónicas mediante la transformación de Fourier;

iii. Calcular la velocidad de grupo de un sistema dispersivo;

iv. Comprender las relaciones entre una función y su transformada de Fourier que llevan a la relación entre ancho de banda y fidelidad;

v. Ser capaz de describir la dispersión de un paquete de ondas;

vi. Comprender el efecto de las cargas libres sobre la relación de dispersión de las ondas electromagnéticas.
